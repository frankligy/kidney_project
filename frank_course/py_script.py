...
# multiple testing correction for log-rank test
p_value_col = []
for celltype in combine.columns[0:14]:
    sorted_combine = combine.sort_values(by=celltype)
    low = sorted_combine.iloc[:200, :].loc[:, ['OS', 'OS.time']]
    high = sorted_combine.iloc[-200:, :].loc[:, ['OS', 'OS.time']]
    results = logrank_test(low['OS.time'], high['OS.time'], low['OS'], high['OS'])
    p_value_col.append(results.p_value)
import statsmodels.stats.multitest as ssm
rejected,p_adj = ssm.fdrcorrection(p_value_col)
final = np.column_stack([p_value_col,p_adj])
...
# proportion changes as progression
phenotype = pd.read_csv('/Users/ligk2e/Desktop/tmp/kidney/xena/phenotype.txt',sep='\t',index_col=0)['pathologic_stage'].to_frame()
new = pd.concat([final,phenotype],axis=1,join='inner')
new = new.loc[new['pathologic_stage']!='[Discrepancy]',:]
new.sort_values(by='pathologic_stage',inplace=True)
sns.pointplot(x='pathologic_stage',y='ccRCC',data=new)
plt.savefig('/Users/ligk2e/Desktop/tmp/kidney/xena/proportion_ccRCC.pdf',bbox_inches='tight')
